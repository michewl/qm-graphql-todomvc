//! Runs some tests for the test infrastructure.
//!
//! The setup tests are mainly for demonstration purposes.
//!
//! Run with `cargo test --test setup`
...
async fn main() {
    ...
    // Run all tests except the ones tagged `@smoketest`
    AppWorld::cucumber()
        .fail_on_skipped()
        ...
        .filter_run("tests/features", |feature, _rule, scenario| {
            let tags: Vec<String> = feature
                ...
                .cloned()
                .collect();
            has_tag_condition(&tags, &vec![String::from("setup")], true)
        })
        .await;
}
